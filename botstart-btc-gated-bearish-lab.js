...
function main() {
  ...
  if (existingPid) {
    console.log(`- btc-gated-bearish-lab (pid ${existingPid}): RUNNING`);
    return;
  }

  const pid = startProcess("bash", [
    path.join(__dirname, "runtime", "run-btc-gated-bearish-lab-loop.sh"),
  ]);
  fs.writeFileSync(PID_FILE, String(pid), "utf8");

  console.log(`- btc-gated-bearish-lab (pid ${pid}): RUNNING`);
}
