...
class Binance {
    ...
    wsApiConnections = {}; // WebSocket API connections
    wsApiPendingRequests = {}; // Pending JSON-RPC requests
    futuresInfo = {};
    futuresMeta = {};
    futuresTicks = {};
    futuresRealtime = {};
    ...
    assignOptions(opt = {}) {
        ...
        if (this.Options.httpsProxy)
            this.httpsProxy = this.Options.httpsProxy;
    }
    ...
    getWsApiUrl() {
        if (this.Options.test)
            return this.wsApiTest;
        return this.wsApi;
    }
    getDStreamSingleUrl() {
        if (this.Options.demo)
            return this.dstreamSingleDemo;
        if (this.Options.test)
            return this.dstreamSingleTest;
        return this.dstreamSingle;
    }
    getFStreamSingleUrl() {
        if (this.Options.demo)
            return this.fstreamSingleDemo;
        if (this.Options.test)
            return this.fstreamSingleTest;
        return this.fstreamSingle;
    }
    getFStreamUrl() {
        if (this.Options.demo)
            return this.fstreamDemo;
        if (this.Options.test)
            return this.fstreamTest;
        return this.fstream;
    }
    getDStreamUrl() {
        if (this.Options.demo)
            return this.dstreamDemo;
        if (this.Options.test)
            return this.dstreamTest;
        return this.dstream;
    }
    uuid22(a) {
    ...
    requireApiSecret(source = 'requireApiSecret', fatalError = true) {
        ...
        if (!this.APISECRET && !this.PRIVATEKEY) {
            if (fatalError)
                throw Error(`${source}: Invalid API Secret or Private Key!`);
            return false;
        }
        return true;
    }
    ...
    async signedRequest(url, data = {}, method = 'GET', noDataInSignature = false) {
        ...
        if (method === 'POST') {
            ...
            if (query)
                encodedUrl += '?' + query;
            if (signature)
                encodedUrl += '&signature=' + signature;
            const opt = this.reqObj(encodedUrl, data, method, this.APIKEY);
...
*/
    ...
    */
     ...
     * @returns
     */
    async futuresAlgoOrder(type, side, symbol, quantity, price, params = {}) {
        params.symbol = symbol;
        params.side = side;
        params.type = type;
        if (quantity)
 ...
 */
    ...
    */
     ...
     */
    connectWsApi(connectionId, messageHandler, reconnect) {
        const httpsproxy = this.getHttpsProxy();
        let socksproxy = this.getSocksProxy();
        let ws = undefined;
        if (socksproxy) {
            socksproxy = this.proxyReplacewithIp(socksproxy);
            if (this.Options.verbose)
                this.Options.log('WebSocket API: using socks proxy server ' + socksproxy);
            const agent = new socksProxyAgent.SocksProxyAgent({
                protocol: this.parseProxy(socksproxy)[0],
                host: this.parseProxy(socksproxy)[1],
                port: this.parseProxy(socksproxy)[2]
            ...
            const agent = new httpsProxyAgent.HttpsProxyAgent(config);
            if (this.Options.verbose)
                this.Options.log('WebSocket API: using proxy server ' + agent);
            ws = new WebSocket(this.getWsApiUrl(), { agent: agent });
        }
        else {
        ...
        ws.on('open', () => {
            if (this.Options.verbose)
                this.Options.log('WebSocket API: Connected to ' + this.getWsApiUrl());
            ws.isAlive = true;
        });
        ws.on('pong', () => { ws.isAlive = true; });
        ws.on('error', (err) => {
            this.Options.log('WebSocket API error: ' + ws.connectionId + ' error: ' + err.message);
        });
        ws.on('close', (code, reason) => {
            if (this.Options.verbose)
                this.Options.log('WebSocket API closed: ' + ws.connectionId +
                    (code ? ' (' + code + ')' : '') +
                    (reason ? ' ' + reason : ''));
            delete this.wsApiConnections[connectionId];
            if (ws.reconnect && typeof reconnect === 'function') {
                if (this.Options.verbose)
                    this.Options.log('WebSocket API reconnecting: ' + connectionId);
                reconnect();
            }
        });
        ws.on('message', data => {
            try {
                if (this.Options.verbose)
                    this.Options.log('WebSocket API data:', data);
     ...
     */
    sendWsApiRequest(connectionId, method, params = {}) {
        return new Promise((resolve, reject) => {
            ...
            this.wsApiPendingRequests[requestId] = { resolve: (v) => settle(resolve, v), reject: (e) => settle(reject, e), timer, connectionId };
            if (this.Options.verbose) {
     ...
     * @return {undefined}
     */
    terminateWsApi(connectionId, reconnect = false) {
        if (this.Options.verbose)
            this.Options.log('WebSocket API terminating:', connectionId);
        const ws = this.wsApiConnections[connectionId];
        if (!ws)
            return;
        ws.removeAllListeners('message');
   ...
   */
    ...
    userDataHandler(data) {
        ...
        if (type === 'eventStreamTerminated') {
            ...
            if (this.Options.all_updates_callback)
                this.Options.all_updates_callback(eventData);
            return;
        }
        if (this.Options.all_updates_callback)
            this.Options.all_updates_callback(eventData);
        if (type === 'outboundAccountInfo') ;
            ...
            if (this.Options.execution_callback)
                this.Options.execution_callback(eventData);
        }
        else if (type === 'listStatus') {
            if (this.Options.list_status_callback)
                this.Options.list_status_callback(eventData);
        }
        else if (type === 'outboundAccountPosition' || type === 'balanceUpdate') {
            if (this.Options.balance_callback)
                this.Options.balance_callback(eventData);
        }
    ...
    userMarginDataHandler(data) {
        ...
        if (type === 'eventStreamTerminated') {
            ...
            if (this.Options.margin_all_updates_callback)
                this.Options.margin_all_updates_callback(eventData);
            return;
        }
        if (this.Options.margin_all_updates_callback)
            this.Options.margin_all_updates_callback(eventData);
        if (type === 'outboundAccountInfo') ;
            ...
            if (this.Options.margin_execution_callback)
                this.Options.margin_execution_callback(eventData);
        }
        else if (type === 'listStatus') {
            if (this.Options.margin_list_status_callback)
                this.Options.margin_list_status_callback(eventData);
        }
        else if (type === 'outboundAccountPosition' || type === 'balanceUpdate') {
            if (this.Options.margin_balance_callback)
                this.Options.margin_balance_callback(eventData);
        }
        else if (type === 'externalLockUpdate') {
            // Handle external lock updates (e.g., when balance is locked for margin collateral)
            if (this.Options.margin_balance_callback)
   ...
   */
    ...
    */
    async tickerPrice(symbol, symbols, options = {}) {
        if (symbol && symbols) {
    ...
    */
    ensureWsApiConnection(connectionId) {
        return new Promise((resolve, reject) => {
    ...
    * @return {promise or undefined} - omitting the callback returns a promise
    */
    async withdraw(coin, address, amount, addressTag, name, params = {}) {
    ...
    */
    ...
    async spotGetDataStream(params = {}) {
        return await this.privateSpotRequest('v3/userDataStream', params, 'POST', true);
    }
    async spotKeepDataStream(listenKey = undefined, params = {}) {
        listenKey = listenKey || this.Options.listenKey;
        if (!listenKey)
            throw new Error('A listenKey is required, either as an argument or in this.Options.listenKey');
        return await this.privateSpotRequest('v3/userDataStream', { listenKey, ...params }, 'PUT');
    }
    async spotCloseDataStream(listenKey = undefined, params = {}) {
        listenKey = listenKey || this.Options.listenKey;
        if (!listenKey)
            throw new Error('A listenKey is required, either as an argument or in this.Options.listenKey');
    ...
    */
     ...
     * @returns
     */
    async futuresSymbolConfig(params = {}) {
    ...
    */
     ...
     * @returns
     */
    async futuresAlgoOrderStatus(symbol, params = {}) {
        params.symbol = symbol;
        return await this.privateFuturesRequest('v1/algoOrder', params);
    }
    /**
     ...
     * @returns
     */
    async futuresCancelAlgoOrder(symbol, orderId, params = {}) {
        params.symbol = symbol;
        if (orderId)
            params.algoid = orderId;
        return await this.privateFuturesRequest('v1/algoOrder', params, 'DELETE');
     ...
     * @returns
     */
    async futuresCancelAllAlgo(symbol, params = {}) {
        params.symbol = symbol;
        return await this.privateFuturesRequest('v1/algoOpenOrders', params, 'DELETE');
    }
    /**
     ...
     * @returns
     */
    async futuresOpenAlgoOrders(symbol, params = {}) {
        if (symbol)
            params.symbol = symbol;
     ...
     * @returns
     */
    async futuresAllAlgoOrders(symbol, params = {}) {
        if (symbol)
            params.symbol = symbol;
 ...
 */
    ...
    userData(all_updates_callback, balance_callback, execution_callback, subscribed_callback, list_status_callback) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.userData(all_updates_callback, balance_callback, execution_callback, subscribed_callback, list_status_callback);
        };
        // Set up callbacks
        ...
        ws.on('open', async () => {
            try {
                ...
                if (subscribed_callback)
                    subscribed_callback(connectionId);
            }
            catch (error) {
    ...
    userMarginData(all_updates_callback, balance_callback, execution_callback, subscribed_callback, list_status_callback) {
        ...
        const reconnect = () => {
            if (this.Options.reconnect)
                this.userMarginData(all_updates_callback, balance_callback, execution_callback, subscribed_callback, list_status_callback);
        };
        // Set up callbacks
        ...
        this.apiRequest(this.sapi + 'v1/userListenToken', {}, 'POST').then((response) => {
                ...
                try {
                    ...
                    if (renewalTime > 0) {
                        setTimeout(async function renewSubscription() {
                            try {
                                // Get new listenToken
                                const renewResponse = await self.apiRequest(self.sapi + 'v1/userListenToken', {}, 'POST');
   ...
   */
    userDeliveryData(margin_call_callback, account_update_callback, order_update_callback, subscribed_callback) {
        ...
        this.apiRequest(url + "v1/listenKey", {}, "POST").then((response) => {
            ...
            const subscription = this.deliverySubscribe(this.Options.listenDeliveryKey, this.userDeliveryDataHandler.bind(this), { reconnect });
            if (subscribed_callback)
                subscribed_callback(subscription.endpoint);
        });
        // }
