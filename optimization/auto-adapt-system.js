...
function decideSymbolConfig(symbol, trades, baseCfg, prevAdaptive, globals) {
  ...
  const bestTpCap = pickBestValue(tpCapStats, globals.minClosedTradesForUpdate, stats.avgR);

  if (bestTpCap) {
    next.requireSr = true;
    reasons.push(`tp_cap_pref_${String(bestTpCap.value)}`);

    if (bestTpCap.value === true) {
      const bufferCandidates = valueStats(
        trades.filter((t) => t.tpCappedByResistance === true),
        "tpCappedByResistance",
        [true]
      );
      if (bufferCandidates.length) {
        next.tpResistanceBufferAtr = clamp(
          next.tpResistanceBufferAtr,
          0.05,
          0.35
        );
      }
...
function main() {
  ...
  const symbols = Array.from(new Set(resolvedTrades.map((t) => t.symbol).filter(Boolean))).sort();
  ...
  if (enabledSymbols.length === 0) {
    ...
    if (fallbackSymbol) {
      ...
      if (historySnapshot.symbols[fallbackSymbol]) {
        historySnapshot.symbols[fallbackSymbol].reasons.push("forced_fallback_enable");
      }
    }
  }
