...
function pickBtcContextProbeStrategy(cfg = {}, selectedStrategy, strategyCandidates) {
  if (selectedStrategy) return selectedStrategy;

  let bestCandidate = null;

  for (const candidate of Array.isArray(strategyCandidates) ? strategyCandidates : []) {
    const strategyName = String(candidate?.strategy || "").trim();
    ...
    const nextScore = Number(candidate?.score ?? -Infinity);
    const bestScore = Number(bestCandidate?.score ?? -Infinity);
    ...
    if (
      nextScore === bestScore &&
      candidate?.signalClass === "EXECUTABLE" &&
      bestCandidate?.signalClass !== "EXECUTABLE"
    ) {
      bestCandidate = candidate;
    }
  }

  return bestCandidate?.strategy || null;
}
...
async function processSymbol(
  ...
  const btcContextProbeStrategy = pickBtcContextProbeStrategy(
    cfg,
    selectedStrategy,
    strategyCandidates
  );
  const executionOverrides = getStrategyExecutionOverrides(
    cfg,
  ...
  const btcContextDecision = btcContextProbeStrategy
    ? await evaluateBtcContextGateForSignal({
        cfg,
        strategyName: btcContextProbeStrategy,
        signalCandleCloseTime: last.closeTime,
        symbolTf,
        marketDataCache,
        enabledCryptoSymbols,
  ...
  if (lastSignalLogEntry) {
    lastSignalLogEntry.btcContextGateStrategy = btcContextProbeStrategy;
    lastSignalLogEntry.btcContextGateApplied = btcContextDecision.applied === true;
    lastSignalLogEntry.btcContextGatePassed = btcContextDecision.passed === true;
    lastSignalLogEntry.btcContextGateReason = btcContextDecision.reason || null;
    lastSignalLogEntry.btcContextState = btcContextDecision.snapshot?.state || null;
    lastSignalLogEntry.btcContextFollowRate = Number.isFinite(
      Number(btcContextDecision.snapshot?.alts?.followRate)
    )
      ? Number(btcContextDecision.snapshot.alts.followRate)
      : null;
    lastSignalLogEntry.btcContextNegativeBreadth = Number.isFinite(
      Number(btcContextDecision.snapshot?.alts?.negativeBreadth)
    )
      ? Number(btcContextDecision.snapshot.alts.negativeBreadth)
      : null;
    lastSignalLogEntry.btcContextSummary = btcContextDecision.snapshot?.summary || null;
  }
