...
// =========================
// Soft trade quality filters
// =========================

const MIN_RR_PLANNED = Number(process.env.MIN_RR_PLANNED || 0.9);
const MIN_TP_DISTANCE_PCT = Number(process.env.MIN_TP_DISTANCE_PCT || 0.0015);
const MIN_TP_ATR_MULT = Number(process.env.MIN_TP_ATR_MULT || 0.35);

const ENTRY_PROJECTION_ATR_MULT = Number(
  process.env.ENTRY_PROJECTION_ATR_MULT || 0.08
);
...
async function processSymbol(symbol, state) {
  ...
  // NOVOS PARAMETROS ADAPTATIVOS
  const MIN_TP_DISTANCE_PCT = Number(cfg.MIN_TP_DISTANCE_PCT ?? 0);
  const MIN_TP_ATR_MULT = Number(cfg.MIN_TP_ATR_MULT ?? 0);
  const ENTRY_PROJECTION_ATR_MULT = Number(cfg.ENTRY_PROJECTION_ATR_MULT ?? 0);
  ...
  const nearEma20 = distToEma20 <= atr * PULLBACK_EMA20_ATR;
  const nearEma50 = distToEma50 <= atr * PULLBACK_EMA50_ATR;
  ...
  if (bullish) score += 10;
  if (bullishFast) score += 10;
  if (stackedEma) score += 10;
  if (nearPullback) score += 10;
  if (nearEma20) score += 10;
  if (rsiInBand) score += 10;
  if (rsiRising) score += 10;
  if (isTrend) score += 10;
  if (isRange) score += 10;
  if (adx >= MIN_ADX) score += 10;

  let signalClass = "IGNORE";
  if (score >= MIN_SCORE) signalClass = "EXECUTABLE";
  else if (score >= MIN_SCORE - 10) signalClass = "WATCH";

  // ======================
  // SUPPORT / RESISTANCE
  // ======================
  const sr = calculateSupportResistance(closedCandles, atr);
  ...
  const distanceToSupportAtr =
    nearestSupport !== null
      ? (last.close - nearestSupport) / atr
      : null;

  const distanceToResistanceAtr =
    nearestResistance !== null
      ? (nearestResistance - last.close) / atr
      : null;
  ...
  if (REQUIRE_SR) {
    if (
      ...
      distanceToResistanceAtr < MIN_SPACE_TO_TARGET_ATR
    ) {
      srPassed = false;
      srReason = "resistance_too_close";
    }

    if (
      distanceToSupportAtr !== null &&
      distanceToSupportAtr > MAX_DISTANCE_FROM_SUPPORT_ATR
    ) {
      srPassed = false;
      srReason = "too_far_from_support";
    }
  }
  ...
  // ENTRY PROJECTION
  // ======================
  const rawEntry = last.close;

  let projectedEntry = rawEntry;
  if (rsiRising && rawEntry >= ema20) {
    projectedEntry = rawEntry + atr * ENTRY_PROJECTION_ATR_MULT;
  }

  // ======================
  // TP / SL
  // ======================
  let sl = projectedEntry - atr * SL_ATR_MULT;
  let tp = projectedEntry + atr * TP_ATR_MULT;

  // TP capped by resistance
  let tpCappedByResistance = false;
  if (nearestResistance) {
    const cappedTp =
      nearestResistance - atr * TP_RESISTANCE_BUFFER_ATR;
    if (cappedTp < tp) {
      tp = cappedTp;
  ...
  const tpDistancePct = (tp - projectedEntry) / projectedEntry;
  const tpDistanceAtr = (tp - projectedEntry) / atr;

  // ======================
  // TP SOFT FILTERS
  // ======================
  let softRejectReason = null;

  if (tpDistancePct < MIN_TP_DISTANCE_PCT) {
    softRejectReason = "tp_too_close_pct";
  }

  if (tpDistanceAtr < MIN_TP_ATR_MULT) {
    softRejectReason = "tp_too_close_atr";
  }
  ...
  console.log(
    `[CORE] ${symbol} close=${last.close.toFixed(
      4
    )} score=${score} class=${signalClass}`
  );
}
  ...
  const entry = round(projectedEntry, 6);
  ...
  state.signalLog.push({
    ...
    entryProjectionAtrMult: ENTRY_PROJECTION_ATR_MULT,
    score,
    signalClass,
    rsi,
    prevRsi,
  ...
  const rrPlanned = safeRatio(reward, risk);
  const tpDistancePct = Math.abs(tp - entry) / entry;
  const tpDistanceAtr = reward / atr;

  if (rrPlanned !== null && rrPlanned < MIN_RR_PLANNED) {
    console.log(
      `[FILTER] ${symbol} bloqueado: RR baixo (${round(
        rrPlanned,
        2
      )} < ${MIN_RR_PLANNED})`
    );
  ...
  if (tpDistancePct < MIN_TP_DISTANCE_PCT) {
    console.log(
      `[FILTER] ${symbol} bloqueado: TP demasiado curto (${round(
        tpDistancePct * 100,
        3
      )}% < ${MIN_TP_DISTANCE_PCT * 100}%)`
    );
    saveState(state);
    return;
  }
  ...
  if (tpDistanceAtr < MIN_TP_ATR_MULT) {
    console.log(
      `[FILTER] ${symbol} bloqueado: TP demasiado curto em ATR (${round(
        tpDistanceAtr,
        2
      )} < ${MIN_TP_ATR_MULT})`
    );
  ...
  const signalObj = {
    ...
    pnlPct: null,
    rrPlanned: rrPlanned,
    tpDistancePct: tpDistancePct,
    tpDistanceAtr: tpDistanceAtr,
    rrRealized: null,
    score,
    signalClass,
  };
  ...
  console.log(
    `[EXECUTOR_CALL] ${symbol} class=${signalClass} score=${score} entry=${entry} tp=${tp} rawTp=${rawTp} capped=${tpCappedByResistance} rr=${round(
      rrPlanned ?? 0,
      2
    )}`
  );
